...
class KD(Distiller):
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        tmp = F.log_softmax(logits_clip_batch, dim=1)
        _, pred_clip = (tmp+0.2*logits_student).topk(1, 1, True, True)
        # _, pred = output.topk(maxk, 1, True, True)
        pred_clip = pred_clip.t()
...
#         loss_kd = loss_kd.mean()
        ...
        loss_clip = min(epoch/20,0.4)*(loss_kd_stu2clip+0.4*loss_kd_stu2tea)
