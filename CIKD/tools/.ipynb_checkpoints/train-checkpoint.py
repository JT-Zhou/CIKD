...
def main(cfg, resume, opts,Cuda_id):
    ...
    # distillation
        ...
        elif cfg.DISTILLER.TYPE=='ICKD':
            data = torch.randn(32, 3, 32, 32)
            with torch.no_grad():
                _,feat_s = model_student(data,is_feat=False)
                _,feat_t = model_teacher(data,is_feat=False)
            # feat_t = feat_t["feats"]
            # feat_s = feat_s["feats"]
            dim = {}
            dim['s_dim'] = feat_s[-2].shape[1]
            dim['t_dim'] = feat_t[-2].shape[1]
            dim['feat_dim'] =feat_t[-2].shape[1]
            distiller = distiller_dict[cfg.DISTILLER.TYPE](
                model_student, model_teacher, cfg, dim
            )
        else:
